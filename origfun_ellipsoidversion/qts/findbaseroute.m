function [cells,locpath,indpath,neighbours]=findbaseroute(start,goal,obst,lims,maxdepth,rad,disttogoal,startdepth,gridsizemult,gridsizepow)
cells=celldecomp(obst,startdepth,lims(1,:),lims(2,:),rad);
...
indpath = cdastar_withnbs(cells,startind,goalind,lims(1,:),lims(2,:),nbs,gridsizemult,gridsizepow);
% figure(3)
% clf
...
%     temppath = cdastar_withnbs(tempcells,startind,goalind,xlims,ylims,nbs_obstless);
    temppath = cdastar_withnbs(tempcells,startind,goalind,lims(1,:),lims(2,:),olnbs,gridsizemult,gridsizepow);
...
% %     hold off
    ocls=newcells;
    [newcells,improvedinds,newinds] = improvepathprec_withnbs(newcells,temppath,obst,lims,maxdepth,rad);
%     neighbours=nbs;
...
%     plot(currx,curry,'r');axis equal;hold off
    indpath = cdastar_withnbs(tempcells,startind,goalind,lims(1,:),lims(2,:),neighbours,gridsizemult,gridsizepow);
