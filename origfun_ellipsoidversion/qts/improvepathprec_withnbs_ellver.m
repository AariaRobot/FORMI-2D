function [newcells,improvedinds,newinds]=improvepathprec_withnbs_ellver(cells,indpath,obstacles,lims,maxdepth,maxrads,M)
obst=obstacles;
newcells=cells;
% maxdepth=maxdepth+1;
improvedinds=[];
...
for ii=1:length(indpath)
    if cells(4,indpath(ii))~=0
        [~,~,extracells] = improveprec_ellver(cells,indpath(ii),obst,lims,maxdepth,maxrads,M);
