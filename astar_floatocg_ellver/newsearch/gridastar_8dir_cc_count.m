function [indpath,cc_count]=gridastar_8dir_cc_count(ocg,startgl,endgl,xdiff,ydiff)
...
cc_count=2;
if size(closedset,2)>0
    cslininds=unique(sub2ind(size(ocg),closedset(1,:),closedset(2,:)));
end
if size(openset,2)>0
    lininds=unique(sub2ind(size(ocg),openset(1,:),openset(2,:)));
%     cc_count=cc_count+length(unique(lininds));
    cc_count=[cslininds,lininds];
end
end
...
nodecost=currnode(5)+distcalc_8dir(cnbind,currnode(1:2),xdiff,ydiff);
if isempty(openset)
